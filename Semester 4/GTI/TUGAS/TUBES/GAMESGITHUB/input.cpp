...
void timer(int value)
{
    if (gameState == STATE_MENU) {
        ...
        timeLeft   = 0;
        isGameOver = true;
        lightR = 0.95f; lightG = 0.60f; lightB = 0.60f;
    }
...
{
    if (gameState == STATE_MENU) {
        if (key == 13 || key == ' ') startGameFromMenu();
        return;
    }

    if (activePuzzle == 0 && !isGameOver && !isGameWon) {
        if (key == 'a' || key == 'A') { deltaAngle = -0.003f; return; }
        if (key == 'd' || key == 'D') { deltaAngle =  0.003f; return; }
        if (key == 'w' || key == 'W') { deltaMove  =  1;      return; }
        if (key == 's' || key == 'S') { deltaMove  = -1;      return; }
    }

    // M -> Manual Book
    if (key == 'm' || key == 'M') { openManualBook(); return; }

    // ESC -> keluar dari puzzle aktif (deselect)
    if (key == 27) {
        activePuzzle = 0;
        glutPostRedisplay();
        return;
    ...
    if (activePuzzle == 0 && key >= '1' && key <= '7') {
        int idx = key - '0';
        if (!puzzleDone[idx]) activePuzzle = idx;
        glutPostRedisplay();
        return;
    }
    ...
    if (key == 'r' || key == 'R') {
        ...
        activePuzzle = 0;
        doorAngle = 0.0f;
        lightR = 0.80f; lightG = 0.85f; lightB = 0.95f;
        feedbackMsg[0] = '\0';
        feedbackTimer  = 0;
        for (int i = 0; i < 8; i++) {
            puzzleDone[i]    = false;
    ...
    if (key == 8) { // Backspace
        if (*curLen > 0) { (*curLen)--; curInput[*curLen] = '\0'; }
    }
        ...
        char lowerInput[16];
        strncpy(lowerInput, curInput, 15); lowerInput[15] = '\0';
        for (int i = 0; lowerInput[i]; i++) lowerInput[i] = tolower(lowerInput[i]);

        char lowerAns[16];
        strncpy(lowerAns, ans, 15); lowerAns[15] = '\0';
        for (int i = 0; lowerAns[i]; i++) lowerAns[i] = tolower(lowerAns[i]);
        ...
        if (strcmp(lowerInput, lowerAns) == 0) {
            ...
            while (next <= 7 && puzzleDone[next]) next++;
...
{
    if (gameState == STATE_MENU) {
        if (key == GLUT_KEY_F1) startGameFromMenu();
        return;
    }
    ...
    switch (key) {
        ...
        case GLUT_KEY_UP:    deltaPitch =  0.003f; break;
        case GLUT_KEY_DOWN:  deltaPitch = -0.003f; break;
        case GLUT_KEY_PAGE_UP:   deltaPitch =  0.003f; break;
        case GLUT_KEY_PAGE_DOWN: deltaPitch = -0.003f; break;
    }
}
...
void releaseKey(int key, int mx, int my)
{
    switch (key) {
        ...
        case GLUT_KEY_PAGE_UP: case GLUT_KEY_PAGE_DOWN: deltaPitch = 0.0f; break;
    }
}
